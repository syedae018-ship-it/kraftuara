/**
 * Centralized Image URL Resolver for External Content & Social Media
 * 
 * Handles transformation of supported external sharing URLs:
 * - Google Drive (sharing links, uc, view, open) -> direct CDN image endpoint
 * - YouTube (watch, youtu.be, shorts, embed) -> high-quality video thumbnail
 * - Instagram (post / reel URLs) -> public CDN media thumbnail
 * - Unsplash / Imgur -> direct high-res image CDN
 * - Direct image links (JPG, PNG, WebP, GIF, SVG, Data URLs) -> pass-through
 */
...
export function resolveProductImageUrl(url?: string | null): string {
  ...
  try {
    ...
    // 2. YouTube Video Links -> HQ Thumbnail
